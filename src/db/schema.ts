...
import { integer, pgTable, text, varchar, uuid, foreignKey, unique, check } from 'drizzle-orm/pg-core';
...
export const decksTable = pgTable('decks', {
  ...
  leaderCardId: varchar({ length: 255 }).notNull().references(() => cardsTable.id, {onDelete: 'cascade'}),
  name: text().notNull(),
  description: text(),
}, (table) => [
  foreignKey({
...
export const deckCardsTable = pgTable('deck_cards', {
  deckId: uuid().notNull().references(() => decksTable.id, {onDelete: 'cascade'}),
  cardId: varchar({ length: 255 }).notNull().references(() => cardsTable.id, {onDelete: 'cascade'}),
  ...
  check('quantity range 1-4', sql`${table.quantity} >= 1 AND ${table.quantity} <= 4`),
]);