...
function parseDeckList(deckList: HTMLElement): Deck {
	...
	if (deckData) {
		...
		if (leaderCardEntry) {
			...
			const leaderCardId = leaderCardEntry.slice(2);
			if (leaderQuantity === 1) {
...
export async function scrapDecks() {
	...
	const deckListHtml = await fetchDeckData(metaUrl.OP07);
	...
	if (!deckListTable) {
		throw new Error('Deck list table not found');
	}
